...
const TOP = new Set(['board_id','content_draft','constraints','persona_set_id']);
...
export async function handler(input, opts={}) {
  ...
  try {
    ...
    if (!personaSet) {
      const g = await personaGen({ board_id, task_context:{ goal:'publish guard', audience:'public', risk_tolerance:'medium', constraints:[], domain:'publish' }, n_personas:5, persona_pack:'comms' }, { statePath });
    ...
    const votes = votesFromContent(personaSet, input.content_draft, input.constraints||{});
    ...
    const pWrite = await writeArtifact(board_id,'persona_set',{ ...personaSet, persona_set_id: crypto.randomUUID(), updated_at: timestamp, lineage:{ parent_persona_set_id: personaSet.persona_set_id }, personas },statePath);
    response.board_writes = [{ type:'decision', success:true, ref:dWrite.ref }, { type:'persona_set', success:true, ref:pWrite.ref }];
    return response;
